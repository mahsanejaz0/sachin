...
const Affiliatereport = () => {
    const [affilateReport, setAffilateReport] = useState([]);
    ...
    const AffilateReport = () => {
        let params = {
            type:"referralbonus"
        }
        postRequest('/report',params,(response) => {
            setAffilateReport(response?.data?.data?.entries);
    ...
    const rowsWithIndex = affilateReport?.map((row, index) => ({ ...row, id: index + 1 }));
